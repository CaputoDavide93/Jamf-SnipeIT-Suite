...
# ── the diagrams ──────────────────────────────────────────────────────────
# Jamf-SnipeIT Suite: one Fargate task definition, fired by five EventBridge
# rules, reconciling Jamf Pro and Snipe-IT against Azure AD and HiBob.
...
def architecture(scheme):
    ...
    rows = 5
    tx, ty = XS[1], TOP
    th = rows * H + (rows - 1) * (PITCH - H)
    ay = ty + th + 72                       # Anthropic, under the task
    k = Canvas(1180, ay + H + 26 + 52, scheme,
               "Five EventBridge rules run one ECS Fargate task that pulls its image from ECR, "
               "keeps secrets and the run lock in SSM, caches AI decisions in S3, logs to "
               "CloudWatch with SNS failure alerts, reads and writes Jamf Pro and Snipe-IT, "
               "only reads Azure AD and HiBob, reports to Slack and asks the Anthropic API "
               "about ambiguous matches.")
    c = k.c
    left = [
        ("EventBridge", ["5 cron rules, one task def,", "each passes its RUN_MODE"],
         "clock", "RunTask + RUN_MODE", "in"),
        ("ECR", ["image :latest, patched", "weekly from the :base tag"],
         "box", "pull :latest", "in"),
        ("SSM Parameter Store", ["API secrets as SecureString", "and the RunMutex lock"],
         "key", "secrets, RunMutex", "both"),
        ("S3 AI cache", ["ai-resolver-cache.json,", "survives Fargate restarts"],
         "bucket", "AI decisions", "both"),
        ("CloudWatch + SNS", ["container logs; failed or", "unlaunched runs alert"],
         "bell", "logs, exit status", "out"),
    ]
    right = [
        ("Jamf Pro", ["computers, local accounts,", "Snipe-IT user info pushed"],
         "laptop", "devices, accounts", "both"),
        ("Snipe-IT", ["users, assets, accessories:", "the register kept true"],
         "tag", "users, assets", "both"),
        ("Azure AD / Entra ID", ["read via Microsoft Graph:", "users, groups, leave dates"],
         "shield", "users, groups", "in"),
        ("HiBob", ["HR source of truth,", "read-only service user"],
         "users", "people, equipment", "in"),
        ("Slack", ["run summaries, errors,", "ambiguous re-hire queue"],
         "chat", "summaries, queue", "out"),
    ]
    for i, (title, subs, icon, label, way) in enumerate(left):
        y = TOP + i * PITCH
        mid = y + H / 2
        a, b = (XS[0] + W + 8, mid), (tx - 8, mid)
        k.add(k.box(XS[0], y, W, H, title, subs, icon=icon),
              k.edge([b, a] if way == "out" else [a, b], label=label, both=way == "both"))
    for i, (title, subs, icon, label, way) in enumerate(right):
        y = TOP + i * PITCH
        mid = y + H / 2
        a, b = (tx + W + 8, mid), (XS[2] - 8, mid)
        k.add(k.box(XS[2], y, W, H, title, subs, icon=icon),
              k.edge([b, a] if way == "in" else [a, b], label=label, both=way == "both"))

    # The task: the one thing that runs, with the module set RUN_MODE picks from.
    k.add(k.box(tx, ty, W, th, "ECS Fargate task",
                ["linux/amd64, read-only root fs,", "every job holds the RunMutex"],
                icon="server", tone="accent"))
    y = ty + 118
    for head, lines in (
            ("LIFECYCLE", ["rehire-detection, leavers,", "azure-starters,", "user-enrichment"]),
            ("SYNC", ["user-match, correction,", "snipe-to-jamf, model-sync,",
                      "peripherals-sync"]),
            ("MAINTENANCE", ["cleanup, health-check,", "ai-audit, reconciliation,",
                             "monthly-digest,", "pending-reconciliation,",
                             "jamf-location-cleanup"]),
            ("ON DEMAND", ["wakeup, username-standardize"])):
        k.add(k.text(tx + 16, y, head, size=11, colour=c["on_accent"], weight="600",
                     opacity="0.85"))
        for j, line in enumerate(lines):
            k.add(k.text(tx + 16, y + 21 + j * 19, line, size=11.5,
                         colour=c["on_accent"], font=MONO))
        y += 21 + len(lines) * 19 + 16

    k.add(k.box(tx, ay, W, H, "Anthropic API",
                ["resolves ambiguous matches,", "runs the weekly ai-audit"], icon="sparkle"),
          k.edge([(tx + W / 2, ty + th + 8), (tx + W / 2, ay - 8)]),
          k.text(tx + W / 2 + 14, (ty + th + ay) / 2 + 4, "ambiguous matches",
                 size=11.5, font=MONO, colour=c["chip"]),
          k.footer("Of the systems of record, only Jamf Pro and Snipe-IT are written to; Azure AD and "
                   "HiBob are read-only. Each rule's RUN_MODE picks the modules."))
    return k.render()

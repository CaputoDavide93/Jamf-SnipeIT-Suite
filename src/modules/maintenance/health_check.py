...
# Generic macOS/system accounts. Org-specific shared accounts are added via
# the matching.skip_usernames config (MATCHING_SKIP_USERNAMES).
SKIP_LOCAL_USERS = {
    "admin", "administrator", "root", "daemon", "nobody",
    "_mbsetupuser", "_spotlight", "guest", "jamfadmin",
}
...
class HealthCheckModule:
    ...
    skip_local_users = SKIP_LOCAL_USERS
    ...
    def __init__(self, config: Config):
        ...
        extra_skip = getattr(getattr(config, "matching", None), "skip_usernames", None) or []
        self.skip_local_users = SKIP_LOCAL_USERS | {
            str(u).strip().lower() for u in extra_skip if str(u).strip()
        }
    ...
    def run(self, dry_run: bool = False) -> Dict[str, Any]:
        ...
        for a in all_assets:
            ...
            for u in lu:
                ...
                if not un or un in self.skip_local_users or un.startswith("_"):
                    continue
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for basic, detail, error in fetched:
                ...
                for local_user in local_users:
                    ...
                    if (
                        not username
                        or username in self.skip_local_users

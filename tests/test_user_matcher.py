...
SNIPE_USERS = [
    {"id": 1, "name": "Peter Grant", "email": "peter.grant@example.com", "username": "peter.grant"},
    {"id": 2, "name": "Paul Grant", "email": "paul.grant@example.com", "username": "paul.grant"},
    {"id": 3, "name": "Jane Sommers", "email": "jane.sommers@example.com", "username": "jane.winters"},  # old username, new name
    {"id": 4, "name": "Alex Morgan", "email": "alex.morgan@example.com", "username": "alex.morgan"},
    {"id": 5, "name": "Alex Morgan", "email": "alex.morgan1@example.com", "username": "alex.morgan1"},
    {"id": 6, "name": "[Disabled] Chris Hale", "email": "chris.hale@example.com", "username": "chris.hale"},
    {"id": 7, "name": "Sam Sample", "email": "sam.sample@example.com", "username": "sam.sample"},
]
...
def test_override_normalised_key():
    ...
    m = make_matcher(overrides={
        "janepersonal": {"snipe_user_id": 1, "snipe_user_name": "Peter Grant", "reason": "test"},
    })
    for variant in ["jane-personal", "jane.personal", "JANEPERSONAL", "jane_personal"]:
...
def test_pick_primary_skips_system_accounts():
    local = [
        ...
        {"name": "examplecorp", "realname": "examplecorp"},
    ]
...
def test_pick_primary_strips_email_domain():
    """Local account 'lucy.grey@example.org' -> username 'lucy.grey'."""
    local = [{"name": "lucy.grey@example.org", "realname": "Lucy Grey"}]
    uname, fullname, _ = pick_primary_local_identity(local)
...
def test_best_match_strips_email_username():
    """best_match called with 'lucy.grey@example.org' should strip
    domain before matching (prevents fuzzy from matching 'Acme' org)."""
    USERS = [
        {"id": 1, "name": "Lucy Grey", "email": "lucy.grey@example.com", "username": "lucy.grey"},
        {"id": 2, "name": "Acme Receivables", "email": "receivables@example.com", "username": "receivables"},
    ]
    m = UserMatcher(users=USERS, email_domain="example.com")
    match, debug = m.best_match(
        full_name_hint="Lucy.grey@example.org",
        username="lucy.grey@example.org",
    )
    assert match is not None
    assert match["id"] == 1
